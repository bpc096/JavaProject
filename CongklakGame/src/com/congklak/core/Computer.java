...
import com.congklak.ui.Gameable;
...
public class Computer extends Player {
	...
	public static String generateName() {
		...
		return comp.computerNames[ Gameable.rand.nextInt(comp.computerNames.length) ];
	}
	...
	protected void simulateTurn(GameState state, ArrayList<Integer> picks, int maxLevel) {
		...
		while (take > 0) {
			...
			if (currentPlayer == state.player && currentIndex == Gameable.BIG_HOLE) {
				++currentIndex;
				continue;
			}
			// change side
			if (currentIndex > Gameable.BIG_HOLE) {
			...
			} else if (currentIndex < Gameable.BIG_HOLE) {
				currentPlayer.setValueHole(currentIndex - 1, currentPlayer.getValueHole(currentIndex - 1) + 1);
			}
			--take;
			// take from last hole
			if (take == 0 && currentIndex < Gameable.BIG_HOLE && currentPlayer.getValueHole(currentIndex - 1) > 1) {
		...
		// take from opponent
		if (currentIndex < Gameable.BIG_HOLE && currentPlayer == state.computer) {
