...
public class GameMenu implements Gameable {
	...
	public GameMenu(MainMenu mainMenu) {
		...
		// player 1
		player1 = new PlayerMenu(mainMenu, PL1);
		player1.createPlayer();
		
		// player 2
		player2 = new PlayerMenu(mainMenu, PL2);
	...
	public GameMenu(MainMenu mainMenu, int level) {
		...
		this.level = level;
		// player
		player1 = new PlayerMenu(mainMenu, PL);
		player1.createPlayer();
		
		// computer
		player2 = new PlayerMenu(mainMenu, CP);
	...
	private void play() {
		while (true) {
			int hole = -1;
			String player1label = PL1; 
			String player2label = PL2;
			...
			printBoard();
			hole = level(currentPlayerMenu, label);
	...
	private int level(PlayerMenu currentPlayerMenu, String label) {
		int hole;
		if (label.contains(PL)) { // is player
			...
			if (comp.pick.isEmpty()) {
				...
				switch (level) {
					case EASY:
						((Computer) comp).combination(state);
						ratio = 0.70;
						break;
					case MEDIUM:
						((Computer) comp).combination(state);
